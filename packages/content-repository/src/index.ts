...
function parsePassage(chunk: string, sourceId: string, normalizedPassageId: string, report: ImportReport): any[] {
  ...
  const lines = chunk.split("\n"); const blocks: any[] = []; let cursor = 0; let ordinal = 0; let passageOrdinal = 0;
  const next = (type: string, value: Record<string, unknown>) => {
    const normalizedValue = value.passageId === normalizedPassageId
      ? { ...value, passageId: ++passageOrdinal === 1 ? normalizedPassageId : `${normalizedPassageId}__${passageOrdinal}` }
      : value;
    blocks.push(typed(type, sourceId, ++ordinal, normalizedValue, report));
  };
