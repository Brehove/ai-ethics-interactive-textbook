...
7. Apply auth migration `0003_mcp_oauth.sql`, deploy the auth Worker, then deploy the textbook MCP Worker. Verify protected-resource and authorization-server discovery advertise `content:live-save`, dynamic public-client registration, PKCE code exchange, refresh rotation, and revocation. Reconnect Codex once without `TEXTBOOK_MCP_ACCESS_TOKEN`, because pre-change refresh grants retain their old scopes. Verify the capability receipt reports `liveSaveMode: trusted-oauth`, then make one controlled D1-authoritative canary edit: `commit_live` must publish directly without `request_live_save_authorization`, while changed base revision, working version, authority, or idempotency preconditions still fail closed. Reuse the same idempotency key and verify it cannot create a second revision. Also retain one automated conformance test for the legacy per-save authorization route.
