...
For schema-v4 drafts, selecting a card opens semantic controls for compact through bleed widths, logical start/center/end alignment, density, adjacent prose wrap, card-text split, and side-by-side card grouping. The editor sends typed operations with the current layout-catalog version; it never persists CSS or pixel measurements. Wraps and splits require adjacent supported prose and preserve the canonical source order when they collapse on mobile or print. Crop framing may be proposed and previewed in a draft, but publishable validation refuses it until the separately protected human frame-approval slice is deployed.
