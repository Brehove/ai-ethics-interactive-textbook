...
Schema v4 retains the single ordered `body` flow and adds renderer-owned presentation. Each layout-capable body card, and each person-feature managed placement, has one semantic `presentation` containing a named width, logical alignment, density, and optional media frame. Multi-node `layoutRegions` decorate one contiguous source span as a wrap, card-text split, or two-to-six-card grid. They never establish a second order.

Every newly migrated v4 document pins `layoutCatalogVersion: "2026-08-06"`; an existing `2026-08-05` v4 document advances deterministically on its next layout mutation. Region IDs are generated by the Content API. Nodes cannot overlap regions, and flow movement or deletion fails until the owning region is removed or reconciled. See [ADR 0009](./architecture/adr/0009-flexible-card-layouts.md).
