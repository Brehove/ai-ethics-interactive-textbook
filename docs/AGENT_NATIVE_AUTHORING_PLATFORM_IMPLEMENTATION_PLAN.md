...
| 2 — control plane/shadow migration | Implemented and remotely seeded: 18 documents/revisions/authority records, semantic operations, CAS, idempotency, audit provenance, human-only review/release, restore-as-draft, paginated passage/dependency reads, and immutable one-time preview issuance | Production Worker/preview deployment; real multi-document merge/rebase, complete graph repositories, and release-lock hardening remain |
| 3 — instructor editor/checkpoints | Chapter 7 write canary plus gated all-book browser selection: the editor loads and losslessly projects every chapter/block type, keeps repository-authoritative chapters read-only, and supports prose/checkpoint/media/embed authoring, review, validation, preview issuance, submit, and exact-snapshot human approve/reject | Real workspace panels, server diff/history/dependency impact UI, remaining block operations, behavioral accessibility tests, and live authenticated smoke test |
...
| 6 — API hardening | Actor provenance, scope separation, CAS/idempotency, exact snapshot verification, semantic diff, reject, restore, authority-gated authoring, paginated passage dependencies, one-time preview grants, and hostile-input tests implemented | Multi-document merge/rebase, serialized deployment-linked release state, generated exhaustive request/response schemas, and broader operational limits |
