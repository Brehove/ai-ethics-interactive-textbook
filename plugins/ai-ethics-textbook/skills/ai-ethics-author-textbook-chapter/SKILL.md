...
- If the user explicitly asked to save or publish the chapter immediately, call `request_live_save_authorization` with the exact current preconditions. Give the user its verification URL and verification code. After the user approves that exact revision, call `commit_live` with the returned `requestId` as `liveSaveRequestId` and the identical changeset, chapter, base revision, expected version, and idempotency key. Report its immutable revision, content hash, projection hash, URL, and whether delivery is verified or pending. If authorization or delivery is pending, report that state; for pending delivery, call `get_live_commit_status`. Never mint a new idempotency key for a retry.
...
Live commit requires a separately issued, instructor-approved capability bound to one exact commit. It creates a public canonical revision and version-history entry, but it does not approve rights, change authority, deploy code/schema, or promote a protected whole-site release.
