...
Use the plugin-provided `ai-ethics-textbook` MCP connection for chapter evidence. If it is unavailable or requires authentication, ask the user to authenticate or reconnect it; do not substitute direct API calls, environment bearers, repository content, or local projections as proof of current live state.
