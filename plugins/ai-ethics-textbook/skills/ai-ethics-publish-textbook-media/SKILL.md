...
Call `preview_changes` after placement. If every pinned media version is cleared and the user explicitly asked to save or publish now, request exact Live Save authorization, show its URL and code, then call `commit_live` with the approved request ID and identical preconditions. Report its delivery state. Otherwise leave the isolated draft. The capability cannot clear rights, change authority, deploy code/schema, promote a protected release, or roll back.
