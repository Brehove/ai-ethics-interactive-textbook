...
5. Choose one terminal action:
   ...
   - Explicit immediate publication: only for one D1-authoritative chapter and only when the user's current request says to save or publish live. Call `commit_live` directly with the exact changeset, chapter, base revision, expected version, and idempotency key. Do not call `request_live_save_authorization`. If trusted publishing is unavailable, ask the user to reconnect the MCP once; never substitute a bypass. If delivery is pending, poll `get_live_commit_status` with the same chapter ID and receipt. Do not create a second commit.

`commit_live` is a chapter-level Save: it creates one immutable public revision and projection, then reports verified or pending delivery. It uses the trusted OAuth `content:live-save` grant and still requires explicit save or publish language from the user. It is not whole-site release approval or promotion. Agents never approve rights, change authority, promote a protected release, or roll back. If `commit_live` is absent, stop rather than substituting a bypass for the user's publication request.
