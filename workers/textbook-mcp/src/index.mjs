...
const presentation = z.object({ width: z.enum(['compact', 'narrow', 'medium', 'reading', 'wide', 'full', 'bleed']), align: z.enum(['start', 'center', 'end']), density: z.enum(['compact', 'standard', 'expanded']), frame: frame.optional() }).strict();
...
export function createMcp(env, requestId, context) {
  ...
  operationTool(server, call, 'set_card_layout', 'Set card size and alignment', 'Set one card semantic width, alignment, density, and optional media frame. Read valid options first.', z.object({ type: z.literal('cardPresentation.set'), cardId: id, presentation, expectedLayoutCatalogVersion }).strict());
