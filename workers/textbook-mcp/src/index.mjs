...
const instructions = 'Read the authoring view, layout catalog, and current passage before editing. Use semantic card widths and typed layout regions; never invent CSS values. Prefer ordinary reading-width cards, wrap only beside sustained prose, split only when card and text form one unit, and grids only for 2-6 peer cards. Create or resume an isolated changeset, preview web/mobile/print, and inspect history. Commit live only when the user explicitly asks to save or publish and the capability explicitly allows commit_live for that exact chapter. Never send raw HTML, CSS, SQL, or database patches.';
...
const gridRegion = z.object({ ...regionBase, type: z.literal('card-grid'), cardNodeIds: z.array(id).min(2).max(6), columns: z.number().int().min(2).max(4), emphasis: z.enum(['equal', 'featured']), featuredNodeId: id.optional() }).strict();
const storedRegion = z.union([wrapRegion.extend({ layoutId: id }), splitRegion.extend({ layoutId: id }), gridRegion.extend({ layoutId: id })]);
...
export function createMcp(env, requestId, context) {
  ...
  operationTool(server, call, 'create_card_group', 'Create side-by-side card group', 'Place two to six peer cards in a responsive two- to four-column grid. This is the normal tool for cards that should sit side by side.', z.object({ type: z.literal('layoutRegion.create'), region: gridRegion, expectedLayoutCatalogVersion }).strict());
