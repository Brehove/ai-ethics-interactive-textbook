...
const operationScopes = Object.freeze({
  get_authoring_view: ['content:read'], get_passage: ['content:read'], get_version_history: ['content:read'], get_live_commit_status: ['content:read'], get_person: ['content:read'], search_persons: ['content:read'],
  create_or_resume_changeset: ['content:write'], replace_passage_text: ['content:write'], replace_chapter_document: ['content:write'], upsert_checkpoint: ['content:write'], remove_checkpoint: ['content:write'], reorder_checkpoint: ['content:write'], place_media: ['content:write'], upsert_embed: ['content:write'], upsert_person_feature: ['content:write'], move_managed_placement: ['content:write'], remove_managed_placement: ['content:write'], preview_changes: ['content:write'], restore_revision_as_draft: ['content:write'], upload_media: ['media:upload'],
...
export function createMcp(env, requestId, context) {
  ...
  operationTool(server, call, 'replace_passage_text', 'Replace passage text', 'Replace prose in one stable passage without raw markup.', z.object({ type: z.literal('text.replace'), passageId: id, text: z.string().min(1).max(50000) }).strict());
