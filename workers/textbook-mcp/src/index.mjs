...
const instructions = 'Use the canonical document ID (for example chapter_ch07), never the public URL slug, in chapterId and documentId fields. Read the authoring view, layout catalog, and current passage before editing. Use semantic card widths, media surfaces, and typed layout regions; never invent CSS values. A media panel is for an image, caption, and credit that form one bounded contextual artifact; ordinary figures remain plain. Prefer ordinary reading-width cards, wrap only beside sustained prose, split only when card and text form one unit, and grids only for 2-6 related cards. Use unequal two-card ratios only when one source-order card is a brief supporting artifact and the other needs the larger reading field. Create or resume an isolated changeset, preview web/mobile/print, and inspect history. Commit live only when the user explicitly asks to save or publish and the capability explicitly allows commit_live for that exact chapter. Never send raw HTML, CSS, SQL, or database patches.';
const id = z.string().min(1).max(200).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/);
const documentId = z.string().regex(/^chapter_[A-Za-z0-9][A-Za-z0-9_-]*$/, 'Use the canonical document ID such as chapter_ch07, not the public slug');
...
const flowPosition = z.union([z.object({ beforeNodeId: id }).strict(), z.object({ afterNodeId: id }).strict()]);
const write = { changeSetId: id, documentId, baseRevisionId: id, expectedVersion: z.number().int().positive(), idempotencyKey: uuid };
const expectedLayoutCatalogVersion = z.string().min(1).max(40);
const frame = z.object({ mode: z.enum(['intrinsic', 'contain', 'crop']), aspect: z.enum(['auto', '1:1', '4:3', '3:2', '16:9', '2:3']).optional(), focalPoint: z.object({ x: z.number().min(0).max(1), y: z.number().min(0).max(1) }).strict().optional() }).strict();
const presentation = z.object({ width: z.enum(['compact', 'narrow', 'medium', 'reading', 'wide', 'full', 'bleed']), align: z.enum(['start', 'center', 'end']), density: z.enum(['compact', 'standard', 'expanded']), surface: z.enum(['plain', 'panel']).optional().describe('Media figures only: panel groups the image, caption, and credit as one bounded contextual artifact.'), frame: frame.optional() }).strict();
const regionBase = { startNodeId: id, endNodeId: id };
const wrapRegion = z.object({ ...regionBase, type: z.literal('wrap'), cardNodeId: id, side: z.enum(['start', 'end']), width: z.enum(['compact', 'narrow', 'medium']) }).strict();
const splitRegion = z.object({ ...regionBase, type: z.literal('card-text-split'), cardNodeIds: z.array(id).min(1).max(3), textNodeIds: z.array(id).min(1).max(20), cardSide: z.enum(['start', 'end']), ratio: z.enum(['card-narrow', 'balanced', 'card-wide']) }).strict();
const gridRegionSchema = (extra = {}) => z.object({ ...regionBase, ...extra, type: z.literal('card-grid'), cardNodeIds: z.array(id).min(2).max(6), columns: z.number().int().min(2).max(4), emphasis: z.enum(['equal', 'featured']), ratio: z.enum(['equal', 'start-narrow', 'end-narrow']).default('equal'), featuredNodeId: id.optional() }).strict().refine((value) => value.ratio === 'equal' || (value.cardNodeIds.length === 2 && value.columns === 2 && value.emphasis === 'equal'), 'Unequal ratios require exactly two cards, two columns, and equal emphasis');
const gridRegion = gridRegionSchema();
const storedRegion = z.union([wrapRegion.extend({ layoutId: id }), splitRegion.extend({ layoutId: id }), gridRegionSchema({ layoutId: id })]);
const checkpoint = z.object({
  checkpointId: id.optional(), legacyId: z.string().min(1).max(200).optional(), passageId: id,
  displayOrder: z.number().int().min(0).optional(), stage: z.string().min(1).max(120).optional(), slotLabel: z.string().min(1).max(120).optional(),
...
const mediaPlacement = z.object({ mediaId: id, mediaVersionId: id, rightsCaseId: id, placementId: id.optional(), anchorPassageId: id, decorative: z.boolean(), alt: z.string().max(2000).optional(), caption: z.string().max(4000).optional(), captionOmissionReason: z.string().max(1000).optional(), teachingUse: z.string().min(1).max(2000), creditOverride: z.string().max(1000).optional(), displayPreset: z.enum(['narrow', 'reading', 'wide', 'bleed']).optional(), align: z.enum(['start', 'center', 'end']).optional(), presentation: presentation.optional(), animationPolicy: z.enum(['clickToPlay', 'playOnce', 'loopWithControls']).optional(), printPolicy: z.enum(['poster', 'firstFrame', 'omit']), downloadable: z.boolean() }).strict().refine((value) => value.decorative || Boolean(value.alt), 'Nondecorative media requires alt text').refine((value) => Boolean(value.caption || value.captionOmissionReason), 'Caption or omission reason is required').refine((value) => Boolean(value.presentation) !== Boolean(value.displayPreset) && Boolean(value.displayPreset) === Boolean(value.align), 'Use either schema-v4 presentation or legacy displayPreset plus align');
...
const externalEmbed = z.object({ kind: z.literal('externalEmbed'), embedId: id.optional(), anchorPassageId: id.optional(), identity: z.object({ provider, resourceType: z.string().min(1).max(80), resourceId: z.string().min(1).max(300), unlistedHash: z.string().min(1).max(200).optional() }).strict(), canonicalUrl: z.string().url().startsWith('https://'), caption: z.string().min(1).max(2000), teachingUse: z.string().min(1).max(2000), displayPreset: z.enum(['compact', 'reading', 'wide']).optional(), presentation: presentation.optional(), theme: z.enum(['light', 'dark', 'auto']), fallback, adapterVersion: z.string().min(1).max(100) }).strict().refine((value) => Boolean(value.presentation) !== Boolean(value.displayPreset), 'Use schema-v4 presentation or legacy displayPreset');
const richLink = z.object({ kind: z.literal('richLink'), linkId: id.optional(), anchorPassageId: id.optional(), canonicalUrl: z.string().url().startsWith('https://'), title: z.string().min(1).max(300), summary: z.string().min(1).max(4000), teachingUse: z.string().min(1).max(2000), linkLabel: z.string().min(1).max(120), posterMediaVersionId: id.optional(), accessedAt: z.string().datetime(), presentation: presentation.optional() }).strict();
const embed = z.union([externalEmbed, richLink]);
...
const managedPlacement = z.object({ placementId: id, kind: z.enum(['personFeature', 'media', 'embed', 'diagram', 'artifact']), contentId: id, anchorPassageId: id, position: z.enum(['before', 'after']).optional(), orderAtAnchor: z.number().int().min(0).optional(), displayPreset: z.enum(['thinker-card', 'narrow', 'reading', 'wide', 'bleed', 'compact']).optional(), presentation: presentation.optional() }).strict().refine((value) => Boolean(value.presentation) !== Boolean(value.displayPreset), 'Use schema-v4 presentation or legacy displayPreset');
...
const operationScopes = Object.freeze({
  get_authoring_view: ['content:read'], get_passage: ['content:read'], get_layout_catalog: ['content:read'], get_card_layout: ['content:read'], get_valid_layout_options: ['content:read'], validate_layout_proposal: ['content:read'], get_version_history: ['content:read'], get_live_commit_status: ['content:read'], get_person: ['content:read'], search_persons: ['content:read'], search_media: ['media:read'], get_media_job: ['media:read'], get_media_asset: ['media:read'],
  create_or_resume_changeset: ['content:write'], replace_passage_text: ['content:write'], replace_chapter_document: ['content:write'], split_block: ['content:write'], join_blocks: ['content:write'], move_block: ['content:write'], upsert_checkpoint: ['content:write'], move_checkpoint: ['content:write'], remove_checkpoint: ['content:write'], reorder_checkpoint: ['content:write'], place_media: ['content:write'], upsert_embed: ['content:write'], resolve_provider_url: ['content:write'], upsert_person_feature: ['content:write'], move_managed_placement: ['content:write'], remove_managed_placement: ['content:write'], set_card_layout: ['content:write'], reset_card_layout: ['content:write'], set_card_frame: ['content:write'], clear_card_frame: ['content:write'], create_card_wrap: ['content:write'], create_card_group: ['content:write'], create_card_text_split: ['content:write'], update_layout_region: ['content:write'], remove_layout_region: ['content:write'], reconcile_layout_region: ['content:write'], preview_changes: ['content:write'], restore_revision_as_draft: ['content:write'], create_media_review_package: ['media:upload'], upload_media: ['media:upload'],
...
export function createMcp(env, requestId, context) {
  ...
  const server = new McpServer({ name: 'ai-ethics-textbook', version: '3.0.0' }, { instructions });
  server.registerTool('get_authoring_view', { title: 'Get authoring view', description: 'Read the complete canonical chapter document, stable passages, checkpoints, placements, and frozen renderer projection. chapterId is the canonical document ID such as chapter_ch07, not the public slug.', inputSchema: { chapterId: documentId }, annotations: readOnly }, async ({ chapterId }) => asResult(await call(`/v1/chapters/${encodeURIComponent(chapterId)}/authoring-view`, { operation: 'get_authoring_view', documentId: chapterId })));
  server.registerTool('get_passage', { title: 'Get passage', description: 'Read one exact stable passage before an anchored edit. Use the canonical chapter document ID.', inputSchema: { chapterId: documentId, passageId: id }, annotations: readOnly }, async ({ chapterId, passageId }) => asResult(await call(`/v1/chapters/${encodeURIComponent(chapterId)}/passages/${encodeURIComponent(passageId)}`, { operation: 'get_passage', documentId: chapterId })));
  server.registerTool('get_layout_catalog', { title: 'Get layout catalog', description: 'Read the current semantic widths, region recipes, responsive rules, print behavior, and usage guidance before proposing a layout.', inputSchema: {}, annotations: readOnly }, async () => asResult(await call('/v1/layout-catalog', { operation: 'get_layout_catalog' })));
  server.registerTool('get_card_layout', { title: 'Get card layout', description: 'Read one card presentation and every layout region that owns it. Use the canonical chapter document ID.', inputSchema: { chapterId: documentId, cardId: id }, annotations: readOnly }, async ({ chapterId, cardId }) => asResult(await call(`/v1/chapters/${encodeURIComponent(chapterId)}/cards/${encodeURIComponent(cardId)}/layout`, { operation: 'get_card_layout', documentId: chapterId })));
  server.registerTool('get_valid_layout_options', { title: 'Get valid layout options', description: 'Get card-specific choices and concise guidance; use this instead of guessing presentation values. Use the canonical chapter document ID.', inputSchema: { chapterId: documentId, cardId: id }, annotations: readOnly }, async ({ chapterId, cardId }) => asResult(await call(`/v1/chapters/${encodeURIComponent(chapterId)}/cards/${encodeURIComponent(cardId)}/layout-options`, { operation: 'get_valid_layout_options', documentId: chapterId })));
  server.registerTool('validate_layout_proposal', { title: 'Validate layout proposal', description: 'Validate a card presentation and/or complete region set without changing the chapter. Use the canonical chapter document ID.', inputSchema: { chapterId: documentId, cardId: id.optional(), presentation: presentation.optional(), layoutRegions: z.array(storedRegion).max(100).optional() }, annotations: readOnly }, async ({ chapterId, ...body }) => asResult(await call(`/v1/chapters/${encodeURIComponent(chapterId)}/layout:validate`, { method: 'POST', operation: 'validate_layout_proposal', documentId: chapterId, body })));
  ...
  operationTool(server, call, 'replace_chapter_document', 'Replace chapter document', 'Replace the editable chapter document while preserving explicit ordered references.', z.object({ type: z.enum(['chapter.replaceDocument', 'chapter.replaceDocumentV3', 'chapter.replaceDocumentV4']), document: z.record(z.unknown()) }).strict(), { ...mutates, destructiveHint: true });
  operationTool(server, call, 'split_block', 'Split prose block', 'Split one schema-v3 prose block. The left fragment retains its identity and the server returns fresh right-fragment identities.', z.object({ type: z.literal('block.split'), blockId: id, offset: z.number().int().positive() }).strict());
  operationTool(server, call, 'join_blocks', 'Join prose blocks', 'Join two adjacent schema-v3 prose blocks without crossing a checkpoint or placement reference.', z.object({ type: z.literal('block.join'), firstBlockId: id, secondBlockId: id }).strict());
  operationTool(server, call, 'move_block', 'Move block', 'Move one block relative to any explicit flow node.', z.object({ type: z.literal('block.move'), blockId: id, position: flowPosition }).strict());
  operationTool(server, call, 'upsert_checkpoint', 'Upsert checkpoint', 'Create or revise checkpoint content. For schema v3, creation requires a flow position and revision never moves the existing reference.', z.object({ type: z.literal('checkpoint.upsert'), checkpoint, position: flowPosition.optional() }).strict());
  ...
  operationTool(server, call, 'move_checkpoint', 'Move checkpoint', 'Move the existing checkpoint reference in chapter flow and optionally rebind its contextual passage.', z.object({ type: z.literal('checkpoint.move'), checkpointId: id, position: flowPosition, passageId: id.optional() }).strict());
  operationTool(server, call, 'reorder_checkpoint', 'Reorder checkpoint (compatibility alias)', 'Compatibility alias for checkpoint.move. Position comes from explicit chapter flow, never displayOrder.', z.object({ type: z.literal('checkpoint.move'), checkpointId: id, position: flowPosition, passageId: id.optional() }).strict());
  operationTool(server, call, 'place_media', 'Place media', 'Place one immutable, cleared media version at an exact ordered-flow position.', z.object({ type: z.literal('media.place'), placement: mediaPlacement, position: flowPosition.optional() }).strict());
  operationTool(server, call, 'upsert_embed', 'Upsert embed', 'Place an allowlisted provider embed with an authored fallback at an exact ordered-flow position; raw iframe markup is not accepted.', z.object({ type: z.literal('embed.upsert'), embed, position: flowPosition.optional() }).strict());
  ...
  operationTool(server, call, 'upsert_person_feature', 'Upsert person feature', 'Place a frozen curated-person feature and its explicit managed placement; schema-v3 creation requires a flow position.', z.object({ type: z.literal('personFeature.upsert'), feature: personFeatureProjection, placement: managedPlacement, position: flowPosition.optional() }).strict());
  operationTool(server, call, 'move_managed_placement', 'Move managed placement', 'Move a typed managed-placement reference relative to any flow node.', z.object({ type: z.literal('managedPlacement.move'), placementId: id, position: flowPosition, anchorPassageId: id.optional(), displayPreset: z.enum(['thinker-card', 'narrow', 'reading', 'wide', 'bleed', 'compact']).optional() }).strict());
  ...
  operationTool(server, call, 'set_card_layout', 'Set card presentation', 'Set one card semantic width, alignment, density, and optional media frame or surface. Use panel only when valid options expose it for a media figure whose image, caption, and credit form one contextual artifact.', z.object({ type: z.literal('cardPresentation.set'), cardId: id, presentation, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'reset_card_layout', 'Reset card layout', 'Reset one card to the server-defined default presentation for its type.', z.object({ type: z.literal('cardPresentation.reset'), cardId: id, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'set_card_frame', 'Set card media frame', 'Set intrinsic, contain, or crop framing. Crop focal points are normalized from 0 to 1; this tool cannot approve a crop.', z.object({ type: z.literal('cardFrame.set'), cardId: id, frame, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'clear_card_frame', 'Clear card media frame', 'Remove explicit frame settings while preserving card size and alignment.', z.object({ type: z.literal('cardFrame.reset'), cardId: id, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'create_card_wrap', 'Wrap prose beside a card', 'Create a left/start or right/end wrap for one compact, narrow, or medium card beside sustained prose.', z.object({ type: z.literal('layoutRegion.create'), region: wrapRegion, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'create_card_group', 'Create side-by-side card group', 'Place two to six related cards in a responsive grid. Use equal for peers; for exactly two cards use start-narrow or end-narrow only when that source-order card is a brief supporting artifact and the other needs the larger reading field.', z.object({ type: z.literal('layoutRegion.create'), region: gridRegion, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'create_card_text_split', 'Create card-text split', 'Pair one to three cards with a deliberate text span using card-narrow, balanced, or card-wide proportions.', z.object({ type: z.literal('layoutRegion.create'), region: splitRegion, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'update_layout_region', 'Update layout region', 'Revise one server-identified layout region; the complete chapter remains validated atomically.', z.object({ type: z.literal('layoutRegion.update'), layoutId: id, patch: z.record(z.unknown()), expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'remove_layout_region', 'Remove layout region', 'Remove one region without deleting or reordering its cards or prose.', z.object({ type: z.literal('layoutRegion.remove'), layoutId: id, expectedLayoutCatalogVersion }).strict());
  operationTool(server, call, 'reconcile_layout_region', 'Reconcile all layout regions', 'Atomically replace the complete region set after reading the current chapter. Use for coordinated multi-region changes.', z.object({ type: z.literal('layoutRegion.reconcile'), regions: z.array(storedRegion).max(100), expectedLayoutCatalogVersion }).strict());
  server.registerTool('upload_media', { title: 'Request media upload', description: 'Request a short-lived, hash- and size-bound, single-use raw-binary upload ticket. Use its public upload URL and one-time token with the bundled local helper; no standing OAuth bearer is exported.', inputSchema: uploadRequest, annotations: mutates }, async (body) => {
    ...
    return asResult({ ...ticket, upload: { ...ticket.upload, url: `${new URL(MCP_RESOURCE).origin}/media-upload/${encodeURIComponent(ticket.ticketId)}` } });
  });
  server.registerTool('preview_changes', { title: 'Preview changes', description: 'Create a one-time, read-only snapshot preview. Layout changes must be checked wide, narrow, mobile, print, offline, and without JavaScript before live save.', inputSchema: { ...write, surface: z.enum(['web', 'webWide', 'webNarrow', 'mobile', 'print', 'offline', 'noJs']).optional() }, annotations: mutates }, async ({ changeSetId, documentId, baseRevisionId, expectedVersion, idempotencyKey, surface }) => asResult(await call(`/v1/changesets/${encodeURIComponent(changeSetId)}:renderPreview`, { method: 'POST', operation: 'preview_changes', documentId, mutation: true, body: { documentId, baseRevisionId, expectedVersion, idempotencyKey, ...(surface ? { surface } : {}) } })));
