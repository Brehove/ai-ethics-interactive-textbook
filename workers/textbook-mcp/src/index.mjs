...
export function createMcp(env, requestId = crypto.randomUUID(), identity = DEFAULT_IDENTITY) {
  ...
  server.registerTool('render_preview', { title: 'Render Protected Preview', description: 'Freeze the current working version into an immutable five-minute, one-time, read-only preview URL. The URL contains no authoring credential.', inputSchema: { ...write, surface: z.enum(['web', 'mobile', 'print', 'offline']).optional() }, annotations: mutates }, async ({ changeSetId, baseRevisionId, expectedVersion, idempotencyKey, surface }) => asResult(await call(`/v1/changesets/${encodeURIComponent(changeSetId)}:renderPreview`, 'POST', { baseRevisionId, expectedVersion, idempotencyKey, ...(surface ? { surface } : {}) })));
