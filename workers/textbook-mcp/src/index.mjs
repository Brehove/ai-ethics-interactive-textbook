...
const instructions = 'Read chapters and passages first. Create or resume a changeset and make only semantic edits. Validate and inspect the diff before either submitting for human review or, only when the user explicitly requested immediate publication and the capability includes content:live-save, saving one D1-authoritative chapter live. Live save creates an immutable canonical revision; it does not approve or promote a protected whole-site release. Every mutation carries current preconditions and an idempotency key. Never use raw HTML, CSS, SQL, or database patches.';
...
const publishesLive = { readOnlyHint: false, destructiveHint: true, openWorldHint: true, idempotentHint: true };
...
const allowedAgentScopes = new Set([...DEFAULT_IDENTITY.scopes, 'content:live-save']);
...
export function createMcp(env, requestId = crypto.randomUUID(), identity = DEFAULT_IDENTITY) {
  const call = api(env, requestId, identity); const server = new McpServer({ name: 'ai-ethics-textbook', version: '0.3.0' }, { instructions });
  ...
  server.registerTool('save_live_revision', { title: 'Save and Publish Live Revision', description: 'Immediately make one validated D1-authoritative chapter changeset the canonical public revision. Use only when the user explicitly requests live save or publication. This creates immutable version history but does not approve or promote a protected whole-site release.', inputSchema: { changeSetId: id, baseRevisionId: id, expectedVersion: z.number().int().positive(), idempotencyKey: uuid }, annotations: publishesLive }, async ({ changeSetId, ...body }) => asResult(await call(`/v1/changesets/${encodeURIComponent(changeSetId)}:saveLive`, 'POST', body)));
  const scopeByTool = {
    ...
    create_media_review_package: ['media:upload', 'content:write'], search_media: ['media:read', 'content:read'], get_media_job: ['media:read', 'content:read'], get_media_asset: ['media:read', 'content:read'], diff_changeset: ['content:read'], submit_changeset: ['content:submit'], save_live_revision: ['content:live-save']
  };
  for (const [name, tool] of Object.entries(server._registeredTools)) if (!hasScope(identity, ...(scopeByTool[name] || []))) tool.disable();
  server.registerResource('Agent capability receipt', 'textbook://capabilities', { title: 'Agent capability receipt', description: 'Current authenticated agent identity, scopes, and hard authority boundary.', mimeType: 'application/json' }, async (uri) => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify({ actorId: identity.actorId, clientId: identity.clientId, runId: identity.runId, scopes: identity.scopes, expiresAt: identity.expiresAt || null, maySaveLive: identity.scopes.includes('content:write') && identity.scopes.includes('content:live-save'), cannot: ['approve', 'reject', 'promote a protected whole-site release', 'change authority', 'write D1 or R2 directly'] }) }] }));
