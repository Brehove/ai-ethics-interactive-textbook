...
const managedPlacement = z.object({ placementId: id, kind: z.enum(['personFeature', 'media', 'embed', 'diagram', 'artifact']), contentId: id, anchorPassageId: id, position: z.enum(['before', 'after']).optional(), orderAtAnchor: z.number().int().min(0).optional(), displayPreset: z.enum(['thinker-card', 'narrow', 'reading', 'wide', 'bleed', 'compact']) }).strict();
...
const operationScopes = Object.freeze({
  ...
  create_or_resume_changeset: ['content:write'], replace_passage_text: ['content:write'], replace_chapter_document: ['content:write'], split_block: ['content:write'], join_blocks: ['content:write'], move_block: ['content:write'], upsert_checkpoint: ['content:write'], move_checkpoint: ['content:write'], remove_checkpoint: ['content:write'], reorder_checkpoint: ['content:write'], place_media: ['content:write'], upsert_embed: ['content:write'], resolve_provider_url: ['content:write'], upsert_person_feature: ['content:write'], move_managed_placement: ['content:write'], remove_managed_placement: ['content:write'], preview_changes: ['content:write'], restore_revision_as_draft: ['content:write'], create_media_review_package: ['media:upload'], upload_media: ['media:upload'],
...
export function createMcp(env, requestId, context) {
  ...
  const server = new McpServer({ name: 'ai-ethics-textbook', version: '2.0.0' }, { instructions });
  ...
  operationTool(server, call, 'replace_chapter_document', 'Replace chapter document', 'Replace the editable chapter document while preserving explicit ordered references.', z.object({ type: z.enum(['chapter.replaceDocument', 'chapter.replaceDocumentV3']), document: z.record(z.unknown()) }).strict(), { ...mutates, destructiveHint: true });
