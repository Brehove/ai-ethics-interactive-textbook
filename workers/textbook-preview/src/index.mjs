...
const render = (snapshot) => {
  ...
  for (const item of chapter.checkpoints || []) checkpoints.set(item.passageId, [...(checkpoints.get(item.passageId) || []), item]);
  const body = (chapter.body || []).map((block) => `${renderBlock(block)}${block.passageId ? (checkpoints.get(block.passageId) || []).map((item) => `<aside class="checkpoint"><strong>${esc(item.title)}</strong><p>${esc(item.prompt)}</p><small>${esc(item.slot)} · ${esc(item.minWords)}–${esc(item.maxWords)} words${item.showInSidebar ? ' · sidebar' : ''}</small></aside>`).join('') : ''}`).join('');
