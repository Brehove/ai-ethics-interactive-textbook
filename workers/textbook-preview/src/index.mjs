...
const renderBlock = (block) => {
  const id = block.sectionId ? ` id="${esc(block.sectionId)}"` : '';
  if (block.type === 'heading') return `<h${Math.min(6, Math.max(2, Number(block.level) || 2))}${id}>${esc(block.text)}</h${Math.min(6, Math.max(2, Number(block.level) || 2))}>`;
  if (block.type === 'paragraph') return `<p${id}>${esc(block.text)}</p>`;
  if (block.type === 'blockquote') return `<blockquote${id}>${esc(block.text)}</blockquote>`;
  if (block.type === 'callout') return `<aside class="callout"${id}>${esc(block.text)}</aside>`;
  if (block.type === 'list') { const tag = block.ordered ? 'ol' : 'ul'; return `<${tag}${id}>${(block.items || []).map((item) => `<li>${esc(item)}</li>`).join('')}</${tag}>`; }
  if (block.type === 'table') return `<div class="table-wrap"><table${id}>${(block.rows || []).map((row, index) => `<tr>${row.map((cell) => `<${index === 0 ? 'th' : 'td'}>${esc(cell)}</${index === 0 ? 'th' : 'td'}>`).join('')}</tr>`).join('')}</table></div>`;
  if (block.type === 'codeBlock') return `<pre${id}><code>${esc(block.code)}</code></pre>`;
  if (block.type === 'mediaFigure') return `<figure${id}><div class="media-placeholder">Media preview · ${esc(block.mediaVersionId)}</div>${block.caption ? `<figcaption>${esc(block.caption)}</figcaption>` : ''}</figure>`;
  if (block.type === 'externalEmbed' || block.type === 'richLink') return `<aside class="embed-fallback"${id}><strong>${esc(block.title || block.fallback?.title || 'External resource')}</strong><p>${esc(block.summary || block.fallback?.summary || block.teachingUse || '')}</p><p><a href="${esc(block.canonicalUrl)}" rel="noopener noreferrer">${esc(block.linkLabel || block.fallback?.linkLabel || 'Open canonical source')}</a></p></aside>`;
  if (block.type === 'diagram') return `<figure${id}><div class="media-placeholder">Diagram · ${esc(block.diagramId)}</div><figcaption>${esc(block.alt)}</figcaption></figure>`;
  if (block.type === 'legacyMarkup') return `<aside class="locked">Locked legacy block. Convert through the reviewed migration path before publication.</aside>`;
  return `<aside class="unknown">Unsupported draft block: ${esc(block.type)}</aside>`;
};

const render = (snapshot) => {
  const chapter = snapshot.chapter || {}; const surface = snapshot.surface || 'web';
  const checkpoints = new Map((chapter.checkpoints || []).map((item) => [item.passageId, item]));
  const body = (chapter.body || []).map((block) => `${renderBlock(block)}${block.passageId && checkpoints.has(block.passageId) ? `<aside class="checkpoint"><strong>${esc(checkpoints.get(block.passageId).title)}</strong><p>${esc(checkpoints.get(block.passageId).prompt)}</p><small>${esc(checkpoints.get(block.passageId).slot)} · ${esc(checkpoints.get(block.passageId).minWords)}–${esc(checkpoints.get(block.passageId).maxWords)} words${checkpoints.get(block.passageId).showInSidebar ? ' · sidebar' : ''}</small></aside>` : ''}`).join('');
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Draft preview · ${esc(chapter.title || snapshot.documentId)}</title><style>:root{color-scheme:light;--ink:#19231d;--muted:#607067;--rule:#d8dfda;--paper:#fff;--accent:#275d46}*{box-sizing:border-box}body{margin:0;background:#eef1ee;color:var(--ink);font:17px/1.65 ui-serif,Georgia,serif}.banner{position:sticky;top:0;z-index:2;padding:.7rem 1rem;background:#14251d;color:#fff;font:600 14px/1.3 ui-sans-serif,system-ui}.meta{opacity:.75;margin-left:.5rem}main{width:min(760px,calc(100% - 2rem));margin:1rem auto 4rem;padding:clamp(1.25rem,5vw,4rem);background:var(--paper);box-shadow:0 12px 42px #16302218}h1,h2,h3{line-height:1.2;margin:2rem 0 .7rem}p,li{max-width:70ch}blockquote,.callout,.checkpoint,.embed-fallback,.locked,.unknown{border-left:4px solid var(--accent);padding:.8rem 1rem;margin:1.5rem 0;background:#f4f7f4}.checkpoint{border:1px solid #aec5b8;border-left:4px solid var(--accent)}small{color:var(--muted)}figure{margin:2rem 0}.media-placeholder{min-height:10rem;display:grid;place-items:center;background:#e8eeea;border:1px solid var(--rule);font:600 14px ui-sans-serif,system-ui}.table-wrap{overflow:auto}table{border-collapse:collapse;width:100%}th,td{border:1px solid var(--rule);padding:.5rem;text-align:left}pre{overflow:auto;padding:1rem;background:#17201b;color:#f5f7f5}a{color:#195a3c}@media(max-width:430px){main{width:100%;margin:0;padding:1rem;box-shadow:none}body{font-size:16px}}@media print{.banner{position:static;background:none;color:#000;border-bottom:1px solid #000}body,main{background:#fff;box-shadow:none;margin:0;width:auto}a[href]::after{content:" (" attr(href) ")";font-size:.8em}}</style></head><body data-preview-surface="${esc(surface)}"><div class="banner">Protected draft preview <span class="meta">one-time · noindex · ${esc(surface)} · ${esc(snapshot.snapshotHash || snapshot.contentHash)}</span></div><main><h1>${esc(chapter.title || 'Untitled chapter')}</h1>${body}</main></body></html>`;
};

const securityHeaders = { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'private, no-store, max-age=0', 'x-robots-tag': 'noindex, nofollow, noarchive', 'referrer-policy': 'no-referrer', 'x-content-type-options': 'nosniff', 'permissions-policy': 'camera=(), microphone=(), geolocation=(), payment=()', 'content-security-policy': "default-src 'none'; style-src 'unsafe-inline'; img-src 'self' data:; frame-ancestors https://ethicsandai.your-digital-life.org; base-uri 'none'; form-action 'none'" };
