...
async function getValidLayoutOptions(env, chapterId, cardId) {
  ...
  return json({ chapterId, revisionId: row.current_revision_id, schemaVersion: 4, layoutCatalogVersion: LAYOUT_CATALOG_VERSION, storedLayoutCatalogVersion: chapter.layoutCatalogVersion, cardId, cardType: card.cardType, presentation: { widths: Object.keys(LAYOUT_CATALOG.widths), aligns: ['start', 'center', 'end'], densities: ['compact', 'standard', 'expanded'], frames: ['intrinsic', 'contain', 'crop'], ...(card.cardType === 'mediaFigure' ? { surfaces: Object.keys(LAYOUT_CATALOG.surfaces) } : {}) }, regions: LAYOUT_CATALOG.regions, guidance: { surface: card.cardType === 'mediaFigure' ? 'Use panel when the image, caption, and credit form one bounded contextual artifact; otherwise keep plain.' : undefined, wrap: 'Use only when adjacent prose is long enough to clear the card.', grid: 'Use equal for peers. For exactly two cards, use start-narrow or end-narrow when the source-order start or end card is a brief supporting artifact.', split: 'Use when card and prose form a single deliberate unit.' } });
}
