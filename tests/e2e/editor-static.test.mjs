...
test("editor shell exposes the structured authoring workflow", async () => {
  ...
  assert.match(shell, /Chapter 7 is the only editable D1 canary/);
  assert.match(shell, /disabled=\{chapter\.order !== 7\}/);
  assert.match(shell, /chapter\.order === 7 \? "editable canary" : "read-only"/);
  ...
  assert.match(shell, /\/v1\/documents\/chapter_ch07/);
  assert.match(shell, /\/v1\/schema/);
  assert.match(shell, /\/v1\/chapters\/chapter_ch07\/changesets/);
  ...
  assert.match(shell, /No semantic differences from canonical Chapter 7/);
