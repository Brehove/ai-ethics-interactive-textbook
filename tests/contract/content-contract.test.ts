...
const checkpoint = (slot: string, n: string) => ({ checkpointId: `checkpoint_${n}`, passageId: "passage_01", passageExcerptHash: hash, slot, stage: slot, strategy: "initial-judgment" as const, title: slot, trigger: "Pause.", prompt: "Respond.", guidance: "Be concrete.", responseStructure: "prose" as const, minWords: 30, maxWords: 250, showInSidebar: true, rationale: "Practice judgment." });

test("draft accepts zero or many checkpoints with unique stable keys and IDs", () => { assert.equal(DraftChapterBundleSchema.safeParse({ ...base, status: "draft", checkpoints: [] }).success, true); assert.equal(DraftChapterBundleSchema.safeParse({ ...base, status: "draft", checkpoints: [checkpoint("commit", "1"), checkpoint("work", "2"), checkpoint("reconcile", "3"), checkpoint("follow-up", "4")] }).success, true); assert.equal(DraftChapterBundleSchema.safeParse({ ...base, status: "draft", checkpoints: [checkpoint("commit", "1"), checkpoint("commit", "2")] }).success, false); assert.equal(DraftChapterBundleSchema.safeParse({ ...base, status: "draft", checkpoints: [checkpoint("commit", "1"), checkpoint("work", "1")] }).success, false); });
test("publication accepts any checkpoint count and preserves collection order", () => { assert.equal(PublishableChapterBundleSchema.safeParse({ ...base, status: "approved", checkpoints: [] }).success, true); assert.equal(PublishableChapterBundleSchema.safeParse({ ...base, status: "approved", checkpoints: [checkpoint("reconcile", "1"), checkpoint("commit", "2"), checkpoint("custom-fourth", "3"), checkpoint("work", "4")] }).success, true); });
...
test("frozen OpenAPI is valid JSON and exposes the complete agent and human boundary", () => {
  ...
  assert.deepEqual(spec.components.requestBodies.mutationEnvelope.content["application/json"].schema.properties.operation.properties.type.enum, ["text.replace", "chapter.replaceBody", "block.insert", "block.move", "block.remove", "checkpoint.upsert", "checkpoint.replace", "checkpoint.remove", "embed.upsert", "media.place", "media.remove"]);
