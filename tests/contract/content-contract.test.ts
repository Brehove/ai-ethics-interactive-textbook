...
test("frozen OpenAPI is valid JSON and exposes the complete agent and human boundary", () => {
  ...
  assert.deepEqual(spec.components.requestBodies.mutationEnvelope.content["application/json"].schema.properties.operation.properties.type.enum, ["text.replace", "chapter.replaceDocument", "chapter.replaceBody", "chapter.importPlainText", "block.insert", "block.move", "block.remove", "checkpoint.upsert", "checkpoint.replace", "checkpoint.remove", "embed.upsert", "media.place", "media.remove", "personFeature.upsert", "managedPlacement.move", "managedPlacement.remove"]);
