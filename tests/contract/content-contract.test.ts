...
test("schema v4 requires normalized card presentation and validates contiguous non-overlapping regions", () => {
  ...
  const v4 = { ...base, schemaVersion: 4 as const, layoutCatalogVersion: "2026-08-06.1" as const, status: "draft" as const, body: [...base.body, ...cards], checkpoints: [], managedPlacements: [], personFeatures: [], layoutRegions: [{ layoutId: "layout_pair", type: "card-grid" as const, startNodeId: cards[0].blockId, endNodeId: cards[1].blockId, cardNodeIds: cards.map((item) => item.blockId), columns: 2, emphasis: "equal" as const, ratio: "equal" as const }] };
  assert.equal(DraftChapterBundleSchema.safeParse(v4).success, true);
  assert.equal(DraftChapterBundleSchema.safeParse({ ...v4, layoutRegions: [{ ...v4.layoutRegions[0], ratio: "start-narrow" }] }).success, true);
  assert.equal(DraftChapterBundleSchema.safeParse({ ...v4, layoutRegions: [{ ...v4.layoutRegions[0], emphasis: "featured", featuredNodeId: cards[1].blockId, ratio: "start-narrow" }] }).success, false);
  assert.equal(DraftChapterBundleSchema.safeParse({ ...v4, body: [...base.body, { ...cards[0], presentation: { ...cards[0].presentation, surface: "panel" } }, cards[1]] }).success, false);
  const mediaPanel = { type: "mediaFigure" as const, blockId: "block_media", figureId: "figure_media", mediaId: "media_media", mediaVersionId: "mediaversion_media", rightsCaseId: "rightscase_media", decorative: false, alt: "An illuminated manuscript page.", caption: "A Renaissance manuscript.", teachingUse: "Read as a transmitted artifact.", presentation: { width: "medium" as const, align: "center" as const, density: "standard" as const, surface: "panel" as const }, printPolicy: "poster" as const, downloadable: false };
  assert.equal(DraftChapterBundleSchema.safeParse({ ...v4, body: [...base.body, mediaPanel], layoutRegions: [] }).success, true);
