...
function inspectorHtml() {
  ...
  const presentation = (card.presentation ?? { width: "reading", align: "center", density: "standard" }) as { width: string; align: string; density: string; surface?: string };
  const surfaceControl = bodyCard?.type === "mediaFigure" ? `<label>Surface<select name="surface"><option value="plain" ${!presentation.surface || presentation.surface === "plain" ? "selected" : ""}>Plain figure</option><option value="panel" ${presentation.surface === "panel" ? "selected" : ""}>Context panel</option></select></label><p class="inspector-note">Use a context panel when the image, caption, and credit should read as one bounded artifact.</p>` : "";
...
function bindInspectorEvents() {
  ...
  app.querySelector<HTMLFormElement>("[data-layout-inspector]")?.addEventListener("submit", async (event) => {
    event.preventDefault(); const form = event.currentTarget; const cardId = String(form.dataset.layoutInspector); const values = new FormData(form);
    ...
    try {
      ...
      const current = target.presentation ?? { width: "reading", align: "center", density: "standard" };
      const presentation = { ...current, width: String(values.get("width")), align: String(values.get("align")), density: String(values.get("density")), ...("type" in target && target.type === "mediaFigure" ? { surface: String(values.get("surface") ?? "plain") } : {}) };
      ...
      setState("dirty"); render();
    } catch (error) { setAttention(error, "The card presentation could not be updated."); }
  });
...
function bindForms() {
  ...
  app.querySelector<HTMLFormElement>("[data-media-form]")?.addEventListener("submit", async (event) => {
    ...
    const placement = { mediaId: String(item.id), mediaVersionId: String(item.media_version_id), rightsCaseId: String(item.rights_case_id), anchorPassageId: selectedPassage, decorative: false, alt: String(values.get("alt") ?? "").trim(), caption: String(values.get("caption") ?? "").trim(), teachingUse: String(values.get("teachingUse") ?? "").trim(), ...(chapter.schemaVersion === 4 ? { presentation: { width: String(values.get("displayPreset") ?? "reading"), align: "center", density: "standard", surface: String(values.get("surface") ?? "plain") } } : { displayPreset: String(values.get("displayPreset") ?? "reading"), align: "center" }), animationPolicy: "clickToPlay", printPolicy: "poster", downloadable: false };
