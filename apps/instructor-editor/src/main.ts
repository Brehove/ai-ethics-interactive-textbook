...
function bindForms() {
  ...
  app.querySelector<HTMLFormElement>("[data-inspector-form]")?.addEventListener("submit", (event) => { event.preventDefault(); void (async () => { if (!inspector || inspector.kind !== "checkpoint") return; const item = chapter.checkpoints.find((checkpoint) => checkpoint.checkpointId === inspector.id); if (!item) return; const form = new FormData(event.currentTarget); const requestedPassage = nearestPassage(chapter, String(form.get("passageId") ?? item.passageId)); const passageText = checkpointExcerpt(checkpointAnchorBlock(chapter, requestedPassage)); const excerptHash = requestedPassage === item.passageId ? undefined : await sha256Text(passageText); item.title = String(form.get("title") ?? ""); item.prompt = String(form.get("prompt") ?? ""); item.guidance = String(form.get("guidance") ?? ""); const stage = String(form.get("stage") ?? "").trim(); if (stage) item.stage = stage; else delete item.stage; moveCheckpoint(chapter, item.checkpointId, requestedPassage, Number(form.get("displayOrder") ?? item.displayOrder), excerptHash); selectedPassage = item.passageId; setState(item.title && item.prompt ? "dirty" : "attention"); })().catch((error) => { console.error("Unable to update checkpoint.", error); setState("attention"); }); });
