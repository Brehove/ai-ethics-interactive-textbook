...
function checkpointAnchorOptions(selected: string) {
  ...
  return chapter.body.flatMap((block) => {
    ...
    seen.add(passageId);
    const description = typeof block.text === "string" ? block.text : Array.isArray(block.items) ? block.items.join(" ") : "";
...
function bindForms() {
  ...
  app.querySelector<HTMLFormElement>("[data-inspector-form]")?.addEventListener("submit", (event) => { event.preventDefault(); void (async () => { if (!inspector || inspector.kind !== "checkpoint") return; const item = chapter.checkpoints.find((checkpoint) => checkpoint.checkpointId === inspector.id); if (!item) return; const form = new FormData(event.currentTarget); const requestedPassage = nearestPassage(chapter, String(form.get("passageId") ?? item.passageId)); const anchor = chapter.body.find((block) => blockPassage(block) === requestedPassage); const passageText = String(anchor?.text ?? anchor?.items?.join(" ") ?? ""); const excerptHash = requestedPassage === item.passageId ? undefined : await sha256Text(passageText); item.title = String(form.get("title") ?? ""); item.prompt = String(form.get("prompt") ?? ""); item.guidance = String(form.get("guidance") ?? ""); item.stage = String(form.get("stage") ?? ""); moveCheckpoint(chapter, item.checkpointId, requestedPassage, Number(form.get("displayOrder") ?? item.displayOrder), excerptHash); selectedPassage = item.passageId; setState(item.title && item.prompt ? "dirty" : "attention"); })().catch((error) => { console.error("Unable to update checkpoint.", error); setState("attention"); }); });
  app.querySelectorAll<HTMLButtonElement>("[data-shift-checkpoint]").forEach((button) => button.addEventListener("click", () => { if (!inspector || inspector.kind !== "checkpoint") return; const item = chapter.checkpoints.find((checkpoint) => checkpoint.checkpointId === inspector.id); if (!item) return; moveCheckpoint(chapter, item.checkpointId, item.passageId, item.displayOrder + Number(button.dataset.shiftCheckpoint ?? 0)); selectedPassage = item.passageId; setState("dirty"); }));
